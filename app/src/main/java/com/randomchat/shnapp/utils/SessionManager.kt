...
class SessionManager(private val context: Context) {
    ...
    // ── Paywall exit offer ────────────────────────────────────────────────────
    private val lastExitOfferShownKey = longPreferencesKey("last_exit_offer_shown_ms")

    /** When the paywall exit offer last showed (0 = never). Rate-limits the offer. */
    val lastExitOfferShownMsFlow: Flow<Long> = context.dataStore.data
        .map { it[lastExitOfferShownKey] ?: 0L }

    suspend fun markExitOfferShown() {
        context.dataStore.edit { it[lastExitOfferShownKey] = System.currentTimeMillis() }
    }

    // ── "Go ad-free" upsell after interstitials ───────────────────────────────
    private val interstitialsTodayKey = intPreferencesKey("interstitials_today")
    private val interstitialsDateKey  = stringPreferencesKey("interstitials_date")
    private val adFreeUpsellDateKey   = stringPreferencesKey("ad_free_upsell_date")

    /**
     * Records an interstitial that was actually shown. Returns true when the
     * "Go ad-free" sheet should follow it: from the [AD_FREE_UPSELL_MIN_INTERSTITIALS]th
     * interstitial of the local day, at most once per day.
     */
    suspend fun recordInterstitialShown(): Boolean {
        var showUpsell = false
        context.dataStore.edit { prefs ->
            val today = todayDateString()
            val count = if (prefs[interstitialsDateKey] == today) (prefs[interstitialsTodayKey] ?: 0) + 1 else 1
            prefs[interstitialsDateKey] = today
            prefs[interstitialsTodayKey] = count
            if (count >= AD_FREE_UPSELL_MIN_INTERSTITIALS && prefs[adFreeUpsellDateKey] != today) {
                prefs[adFreeUpsellDateKey] = today
                showUpsell = true
            }
        }
        return showUpsell
    ...
    companion object {
        ...
        /**
         * Max free rewarded-ad earns per local day. Lowered from 3 to 2 so heavy
         * media users reach the "Upgrade for unlimited" upsell sooner.
         */
        const val DAILY_REWARD_CAP = 2
        /** Interstitials shown in a day before the "Go ad-free" sheet may follow one. */
        const val AD_FREE_UPSELL_MIN_INTERSTITIALS = 2
